...
class RightLineArcLeftView(ctx : Context) : View(ctx) {
    ...
    data class RightLineArcLeft(var i : Int) {

        private var curr : RLALNode = RLALNode(0)
    ...
    data class Renderer(var view : RightLineArcLeftView) {

        private val animator : Animator = Animator(view)
        private val rlal : RightLineArcLeft = RightLineArcLeft(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            rlal.draw(canvas, paint)
            animator.animate {
                rlal.update {
                    animator.stop()
                }
