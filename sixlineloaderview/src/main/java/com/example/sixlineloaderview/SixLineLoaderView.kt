package com.example.sixlineloaderview
...
fun Canvas.drawSixLineLoader(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2) {
        for (j in 0..(parts - 1)) {
            ...
            drawXY(0f, 0f) {
                rotate(rot + rot * j)
                drawLine(0f, 0f, 0f, -size * (scj1 - scj2), paint)
            }
        }
    }
}
...
fun Canvas.drawSLLNode(i : Int, scale : Float, paint : Paint) {
    ...
    drawSixLineLoader(scale, w, h, paint)
}
...
class SixLineLoaderView(ctx : Context) : View(ctx) {
    ...
    data class SLLNode(var i : Int = 0, val state : State = State()) {
    ...
    data class SixLineLoader(var i : Int) {

        private var curr : SLLNode = SLLNode(0)
