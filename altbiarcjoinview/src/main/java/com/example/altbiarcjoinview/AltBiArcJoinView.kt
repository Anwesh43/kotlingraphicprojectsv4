package com.example.altbiarcjoinview
...
fun Canvas.drawAltBiArcJoin(scale : Float, w : Float, h : Float, paint : Paint) {
...
class AltBiArcJoinView(ctx : Context) : View(ctx) {
    ...
    data class AltBiArcJoin(var i : Int) {

        private var curr : ABAJNode = ABAJNode(0)
    ...
    data class Renderer(var view : AltBiArcJoinView) {

        private val animator : Animator = Animator(view)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)
        private val abaj : AltBiArcJoin = AltBiArcJoin(0)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            abaj.draw(canvas, paint)
            animator.animate {
                abaj.update {
                    animator.stop()
                }
