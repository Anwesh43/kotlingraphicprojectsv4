...
class LineHalfArcAlternateView(ctx : Context) : View(ctx) {
    ...
    data class LHAANode(var i : Int = 0, val state : State = State()) {

        private var next : LHAANode? = null
        private var prev : LHAANode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLHAANode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LHAANode {
            var curr : LHAANode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineHalfArcAlternate(var i : Int) {

        private var curr : LHAANode = LHAANode(0)
