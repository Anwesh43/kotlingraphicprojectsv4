package com.example.crosslineintosingleview
...
fun Canvas.drawCrossLineIntoSingle(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2) {
        for (j in 0..1) {
            drawXY(0f, 0f) {
                ...
                drawLine(0f, -size * 0.5f * dsc(j), 0f, size * 0.5f * dsc(j), paint)
            }
        }
    }
}

fun Canvas.drawCLISNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = Color.parseColor(colors[i])
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawCrossLineIntoSingle(scale, w, h, paint)
}

class CrossLineIntoSingleView(ctx : Context) : View(ctx) {
    ...
    data class CrossLineIntoSingle(var i : Int) {

        private var curr : CLISNode = CLISNode(0)
