package com.example.linebentgrowleftview
...
fun Canvas.drawLineBentGrowLeft(scale : Float, w : Float, h : Float, paint : Paint) {
...
fun Canvas.drawLBGLNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = Color.parseColor(colors[i])
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawLineBentGrowLeft(scale, w, h, paint)
}

class LineBentGrowLeftView(ctx : Context) : View(ctx) {
    ...
    data class LineBentGrowLeft(var i : Int) {

        private var curr : LBGLNode = LBGLNode(0)
