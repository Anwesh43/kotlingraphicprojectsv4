package com.example.linerotgodiagview
...
fun Canvas.drawLineRotDiag(scale : Float, w : Float, h : Float, paint : Paint) {
    ...
    drawXY(w / 2, h / 2) {
        for (j in 0..1) {
            drawXY(0f, 0f) {
                scale(1f - 2 * j, 1f)
                drawXY((w / 2) * (1 - dsc(0)) + (w / 2) * dsc(j + 2), -w * 0.5f * dsc(j + 2)) {
                    rotate(-rot * dsc(1))
...
fun Canvas.drawLRGDNode(i : Int, scale : Float, paint : Paint) {
    val w : Float = width.toFloat()
    val h : Float = height.toFloat()
    paint.color = Color.parseColor(colors[i])
    paint.strokeCap = Paint.Cap.ROUND
    paint.strokeWidth = Math.min(w, h) / strokeFactor
    drawLineRotDiag(scale, w, h, paint)
}
...
class LineRotDiagView(ctx : Context) : View(ctx) {
    ...
    companion object {
        fun create(activity : Activity) : LineRotDiagView {
            val view : LineRotDiagView = LineRotDiagView(activity)
