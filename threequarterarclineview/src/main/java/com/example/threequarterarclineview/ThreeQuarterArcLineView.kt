...
class ThreeQuarterArcLineView(ctx : Context) : View(ctx) {
    ...
    data class TQALNode(var i : Int = 0, val state : State = State()) {

        private var next : TQALNode? = null
        private var prev : TQALNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawTQALNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : TQALNode {
            var curr : TQALNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class ThreeQuarterArcLine(var i : Int) {

        private var curr : TQALNode = TQALNode(0)
