...
class ArcLineRotOpenView(ctx : Context) : View(ctx) {
    ...
    data class ArcLineRotOpen(var i : Int) {

        private var curr : ALRONode = ALRONode(0)
    ...
    data class Renderer(var view : ArcLineRotOpenView) {

        private val animator : Animator = Animator(view)
        private val arlo : ArcLineRotOpen = ArcLineRotOpen(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            arlo.draw(canvas, paint)
            animator.animate {
                arlo.update {
                    animator.stop()
                }
