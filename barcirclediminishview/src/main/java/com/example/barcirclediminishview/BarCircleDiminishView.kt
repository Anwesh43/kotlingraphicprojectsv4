package com.example.barcirclediminishview
...
class BarCircleDiminishView(ctx : Context) : View(ctx) {
    ...
    data class BCDNode(var i : Int = 0, val state : State = State()) {

        private var prev : BCDNode? = null
        private var next : BCDNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBCDNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BCDNode {
            var curr : BCDNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BarCircleDiminish(var i : Int) {

        private var curr : BCDNode = BCDNode(0)
