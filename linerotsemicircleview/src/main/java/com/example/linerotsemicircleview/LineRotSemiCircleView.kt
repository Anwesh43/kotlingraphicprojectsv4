...
class LineRotSemiCircleView(ctx : Context) : View(ctx) {
    ...
    data class LRSCNode(var i : Int = 0, val state : State = State()) {

        private var next : LRSCNode? = null
        private var prev : LRSCNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawLRSCNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : LRSCNode {
            var curr : LRSCNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class LineRotSemiCircle(var i : Int) {

        private var curr : LRSCNode = LRSCNode(0)
    ...
    companion object {
        ...
        fun create(activity : Activity) : LineRotSemiCircleView {
            val view : LineRotSemiCircleView = LineRotSemiCircleView(activity)
